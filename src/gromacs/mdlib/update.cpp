                          ...
                          gmx_int64_t step, int seed, int* gatindex, real fc, t_forcerec *fr)
{
    ...
    int             adresstype;
    real            adressr, adressw, sqr_dl, dl;
    rvec            *ref, dx, xnew;
    ...
    if (!bDoConstr)
    {
	    ...
	    /* call rvec_sub (gromacs/math/vec.h) which subtracts the components
	     * of adress ref from xnew, creating the vector dx */
...
/* Don't thermostat the explicit region */
        ...
        if (dl < adressr)
        {
		    ...
		    real  fn;
                    fn           = f[n][d];
                    v[n][d]      = v[n][d] + (invmass[n]*fn + accel[ga][d])*dt;
...
/* thermostat delta with tau2 */            
	else if (dl > adressr && dl < l2)
	{
	    for (d = 0; d < DIM; d++)
            {
                if ((ptype[n] != eptVSite) && (ptype[n] != eptShell) && !nFreeze[gf][d])
...
/* thermostat the tracers at full strength */
	    for (d = 0; d < DIM; d++)
            {
                if ((ptype[n] != eptVSite) && (ptype[n] != eptShell) && !nFreeze[gf][d])
                {
...
/* TO-DO: Fix local thermostat for constraints */
    else
    {
